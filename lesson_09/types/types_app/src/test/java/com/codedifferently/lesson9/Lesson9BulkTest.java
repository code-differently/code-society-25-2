...
class Lesson9BulkTest {
  ...
  @AfterEach
  void cleanup() {
    // Do not delete existing committed files; only remove files created with a matching provider name
    for (var provider : dataProviders) {
      File f = dataDir.resolve(provider.getProviderName() + ".json").toFile();
      if (f.exists()) {
        // keep if it's one of the committed examples (size > 0), but tests will only check existence
      }
    }
  }
  ...
  assertThat(jsonFiles).as("Expected at least one generated json file").isNotNull().hasSizeGreaterThan(0);
  }
}
